...
getClient("http://localhost:3000/api/clients/1652623760402");
...
function createTalbleLine(user) {
  const { id, createdAt, updatedAt, name, surname, lastName, contacts } = user;
  const created = convertISOTime(createdAt);
  const updated = convertISOTime(updatedAt);
  const template = `<tr class="table-body__row">
